...
class ContentWatcherTest extends QuarkusTestBase {
    ...
    void isManagedByOperator() {
        assertTrue(contentWatcher.isManagedByOperator(new DeploymentBuilder().withMetadata(new ObjectMetaBuilder().withLabels(Map.of("managedBy", "spaship-operator")).build()).build()));
    }
